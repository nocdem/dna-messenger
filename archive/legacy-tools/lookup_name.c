...
int main(int argc, char **argv) {
    ...
    strncpy(config.identity, "lookup-client", sizeof(config.identity) - 1);
    ...
    printf("[2/3] Starting DHT node and connecting...\n");
    ...
    // Lookup name
    printf("[3/3] Looking up '%s'...\n", name);
    char *fingerprint = NULL;
    int ret = dna_lookup_by_name(ctx, name, &fingerprint);
    ...
    printf("\n--- Social Links ---\n");
    if (identity->socials.x[0]) printf("X:        %s\n", identity->socials.x);
