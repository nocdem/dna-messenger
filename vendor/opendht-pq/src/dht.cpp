...
{
    // DILITHIUM5: MANDATORY SIGNATURE ENFORCEMENT
    // All values MUST be signed with Dilithium5 (FIPS 204 / ML-DSA-87)
    if (!value->isSigned()) {
        if (logger_)
            logger_->w("[NODUS] Dropping unsigned value (mandatory Dilithium5 signatures)");
        return false;
    }
    ...
    if (!value->checkSignature()) {
        if (logger_)
            logger_->w("[NODUS] Dropping value with invalid Dilithium5 signature");
        return false;
    }
