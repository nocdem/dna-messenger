...
int messenger_delete_identity(messenger_context_t *ctx, const char *identity) {
    ...
    printf("\n[Deleting local identity '%s']\n", identity);

    // NOTE: We do NOT delete from keyserver because:
    // - Other users need the public key to verify old messages
    // - Prevents identity name from being re-claimed
    // - Public keys should remain permanently for message verification

    // Delete local key files only
    const char *home = qgp_platform_home_dir();
    if (!home) {
        fprintf(stderr, "Error: Cannot get home directory\n");
        return -1;
    ...
    char dilithium_path[512];
    snprintf(dilithium_path, sizeof(dilithium_path), "%s/%s-dilithium.pqkey", dna_dir, identity);
    if (remove(dilithium_path) == 0) {
        printf("✓ Deleted signing key: %s\n", dilithium_path);
    } else {
        fprintf(stderr, "Warning: Could not delete signing key: %s\n", dilithium_path);
    }

    // Delete encryption key
    char kyber_path[512];
    snprintf(kyber_path, sizeof(kyber_path), "%s/%s-kyber512.pqkey", dna_dir, identity);
    if (remove(kyber_path) == 0) {
        printf("✓ Deleted encryption key: %s\n", kyber_path);
    } else {
        fprintf(stderr, "Warning: Could not delete encryption key: %s\n", kyber_path);
    }

    // Delete public key bundle
    char pubkey_path[512];
    snprintf(pubkey_path, sizeof(pubkey_path), "%s/%s.pub", dna_dir, identity);
    if (remove(pubkey_path) == 0) {
        printf("✓ Deleted public key bundle: %s\n", pubkey_path);
    } else {
        fprintf(stderr, "Warning: Could not delete public key bundle: %s\n", pubkey_path);
    }

    printf("\n✓ Local identity '%s' deleted (keyserver entry preserved)\n", identity);
    return 0;
}
